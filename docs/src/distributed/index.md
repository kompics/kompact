...
Path-based dispatching lives in `kompact` behind the `distributed` feature. If you want the provided socket transport, use the companion crate `kompact-net`, which supplies the `NetworkDispatcher`, `NetworkConfig`, and related transport APIs on top of the same distributed core.
