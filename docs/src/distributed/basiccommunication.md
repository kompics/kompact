...
In order to use remote communication with Kompact we need the provided transport from `kompact-net`. Path-based dispatch itself lives in `kompact`, but the socket-backed `NetworkDispatcher` and its `NetworkConfig` are now supplied by `kompact-net`. Custom dispatchers in general are set with the `KompactConfig::system_components(...)` function, which also allows replacement of the system's deadletter box, that is the component that handles messages where no recipient could be resolved. An instance of the provided `NetworkDispatcher` should be created via its configuration struct using `NetworkConfig::build()`. The default implementation will bind to `127.0.0.1` on a random free port. Attempting to bind on an occupied port, or without appropriate rights on a reserved port such as 80 will cause the creation of the `KompactSystem` instance to fail.
...
In order to receive distributed messages, a component must implement an appropriate actor interface. The low-level option is to implement `Actor` and handle `receive_network(...)` yourself. If local and distributed messages share the same deserialised type, `NetworkActor` is usually more convenient because it performs the deserialisation step for you. Deserialisation happens lazily in Kompact, which means components receive a `NetMessage` containing the sender path, the serialisation identifier, and the serialised payload. They must then decide whether to deserialise that payload into a concrete message. This can be done using `NetMessage::try_deserialise::<TargetType, Deserialiser>()`, or more conveniently for multiple messages via the `match_deser!` macro. We will get back to serialisation in more detail [later](serialisation.md).
