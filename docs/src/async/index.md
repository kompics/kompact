...
{{#rustdoc_include ../../examples/local/src/bin/dns_resolver.rs:messages}}
```
...
```rust,edition2024,no_run,noplaypen
{{#rustdoc_include ../../examples/local/src/bin/dns_resolver.rs:state}}
```
...
In order to enter the `blocked` state, we return a `HandlerResult` constructed with the `Handled::block_on(...)` convenience method. This method returns `Ok(Handled::BlockOn(...))` and takes the `self` reference to the component and an asynchronous closure, that is a closure that produces a future when invoked. This closure is given a single parameter by the Kompact API, which is an access guard object to a mutable component reference. In other words, a special owned struct that can be mutably dereferenced to the current component definition type. This guard object ensures safe mutable access to the current component instance, whenever the resulting future is polled, but prevents holding on to actual references over `await` calls (which are illegal). It is very important that this guard object is **never** sent to another thread from within the future. The async closure can not directly close over the component's `self` reference, as the correct lifetime for it can not be guaranteed. Only the references obtained from the special guard object are safe in between `await` calls.

The blocking future also completes with a `HandlerResult`, just like a normal message, event, or lifecycle handler. Returning `Handled::OK` unblocks the component and continues normal processing. Returning a `HandlerError` follows the same error handling rules as other handlers.
...
```rust,edition2024,no_run,noplaypen
{{#rustdoc_include ../../examples/local/src/bin/dns_resolver.rs:lifecycle}}
```
...
```rust,edition2024,no_run,noplaypen
{{#rustdoc_include ../../examples/local/src/bin/dns_resolver.rs:actor}}
```
...
```rust,edition2024,no_run,noplaypen
{{#rustdoc_include ../../examples/local/src/bin/dns_resolver.rs:main}}
```
