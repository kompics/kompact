...
impl QuinnEndpoint {
    ...
    pub(super) fn read_incoming(&mut self, remote: SocketAddr){
        let (recv_time, ecn, data) = self.inbound.pop_front().unwrap();
        ...
        self.endpoint
            ...
            //consumes incoming packets and connection-generated events via 
            //handle and handle_event
            .handle(recv_time, remote, None, ecn, data.as_slice().into())
        {
            //Dispatch incoming datagrams to related connection
            match event {
        ...
        for (ch, conn) in self.connections.iter_mut() {
            ...
            for (_, mut events) in self.conn_events.drain() {
                for event in events.drain(..) {
                    ...
                    //Will execute protocol logic upon receipt of a connection event, 
                    //in turn preparing signals (including application Events, EndpointEvents and outgoing datagrams) 
                    //that should be extracted through the relevant methods.
            ...
            while let Some(x) = conn.poll_transmit(now, MAX_DATAGRAMS) {
                //transmitting outgoing packets
                self.outbound.extend(split_transmit(x));
            }
            self.timeout = conn.poll_timeout();

            //Return endpoint-facing events and push it to endpoint_events vector
            while let Some(event) = conn.poll_endpoint_events() {
                endpoint_events.push((*ch, event));
            }
        }
        //Handle events for endpoint and connections from endpoint_events
...
//Certification for setting up QUINN connection
